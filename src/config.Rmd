...
What does it do? Well, as it contains the free identifiers `z` we
...
data types used. This is indeed a very good idea as often the data
type provides enough context for an unambiguous interpretation of
...
[^monad]: While monads are usually implemented by compile-time
    constructs, effect systems resemble exception handling constructs
    which are commonly dynamically scoped.
   
[^dynfun]: Especially, [dynamically scoped functions](https://dl.acm.org/doi/10.1145/944579.944587)
	have been shown to provide similar runtime configuration as discussed in aspect-oriented
	programming.
...
for more general code. In the end, i.e., if every construct could be
extended arbitrarily, without knowing the actual evaluation context,
nothing about the program semantic would be known. For that reason,
most languages include special forms and other language constructs
which cannot be defined or changed by user code. In particular,
compiler passes, such as for type inference or optimizations, can
usually not be changed via user code[^macros].
...
Thus, `using` a library not just defines new types and functions, but
also provides novel contexts for existing generic functions. This is
also the main reason that "type piracy", i.e., defining methods of
generic functions and types from other libraries/modules, should be
avoided as it might break existing code. Similarly, violating
implicitly assumed abstractions might lead to correctness
problems. Despite these issues[^typed], generic programming is very powerful
and often allows to combine functionalities in novel ways, i.e.,
reinterpreting existing programs in novel contexts. As a convinicing
example try
...
dispatch](https://www.youtube.com/watch?v=kc9HwsxE1OY) and embrace
configurable programming in your language of choice.
