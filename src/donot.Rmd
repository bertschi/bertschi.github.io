...
require monads. In any case, desugaring the do--notation allows to
better understand the relation to the functional versions:
...
II. Alternatively, we could just use the definitions for applicative
	functors and require the user to explicitly `join`[^join] the final
	result as it would be of type `m (m r)` then
	
	[^join]: `join :: Monad m => m (m r) -> m r` can be defined for every monad via
		`join mmx = mmx >>= id`.
